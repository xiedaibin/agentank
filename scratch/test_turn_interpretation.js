...
async function testTurnInterpretation() {
    const token = getToken();
    const testCode = `
function onIdle(me, enemy, game) {
    if (game.frames === 1) {
        // We want to force the tank to face left first, or check what happens if we turn left twice.
        // Let's call turn("left") in frame 1, and turn("left") in frame 2, and see if it ends up facing down.
        me.turn("left");
        me.speak("F1");
