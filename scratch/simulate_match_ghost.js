...
async function main() {
    ...
    const matchId = "mat_CempGXxxBdoKbGuhC";
    ...
    meId = raw.replayData.replay.meta.players[meIndex].tank.id;
    ...
    const runCode = new Function('sandbox', `
        with (sandbox) {
            ...
            sandbox.tacticalDefense = tacticalDefense;
    ...
    for (let f = 1; f < records.length; f++) {
        ...
        const enemyObj = {
            tank: enemyVisible ? { id: 392, position: enemyPos.slice(), direction: enemyDir, crashed: false } : null,
        ...
        const meSandboxObj = Object.assign({}, meObj, {
            ...
            teleport: function(x, y) { console.log(`    [DECISION] Teleport to [${x}, ${y}]`); },
        ...
        try {
            ...
            const defAct = sandbox.tacticalDefense(meSandboxObj, ctx);
            console.log(`  tacticalDefense returns:`, JSON.stringify(defAct));
            
            const bestAction = sandbox.tacticalAnalysis(ctx);
            console.log(`  tacticalAnalysis returns:`, JSON.stringify(bestAction));
        } catch (err) {
