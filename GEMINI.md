...
### 7. 对战结果判定与 runTime 结算逻辑（防错铁律）
*   **crashed**：战斗中被子弹摧毁或发生碰撞，属于正常战损，并非代码崩溃。
*   **error**：代码抛出运行异常，属于 Bug，需进行修复。
*   **runTime（平局判定）**：**并非执行超时被杀**，而是在比赛战至最后一帧（128帧结束）且**双方均存活、比分完全相同**时，系统进行的平局裁判——**总运行时间（Runtime）更短的一方获胜**。运行时间较长的一方会在结算中被判定为 `runTime` 负局。因此，优化核心代码结构（如 A* 寻路剪枝）、降低累积运行时间，对确保平局胜出至关重要。
