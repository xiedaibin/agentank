...
function buildExecutionContext(me, enemy, game) {
    ...
    if (!visible && G_History.enemyInvisibleFrames === 1 && currentEnemyPos && currentEnemyDir) {
        ...
        if (!isCloakTank) {
            ...
            if (stepTile === "o") {
                currentEnemyPos = stepTarget; // 纠正为真实的草丛位置
            }
        }
    }
...
function evalPreAim(ctx) {
    ...
    if (isCurrentlyInGrass || recentlyTeleported) {
        if (recentlyTeleported) {
            // 【抢星保护】：如果星星就在我们嘴边（距离仅 1 格），必须优先抢星，禁止预瞄打断吃星
    ...
    if (grass) {
        ...
        if (isCurrentlyInGrass && (!ctx.starPos || starUnsafe)) {
            ...
            if (overloadNearby && isOnEnemyGunLine(ctx.myPos, ctx, true)) {
                ...
                // 金蝉脱壳：进入草丛首帧进行位置转移以欺骗盲射。但如果已经和星星共轴则不转移，方便预瞄与架枪
...
function tacticalDefense(me, ctx) {
    // 【抢星免死金牌防御拦截】：
    // 如果星星在嘴边（距离为 1），且车头正对它，且该吃星路径是安全的，我们直接放行踩星，不触发任何幽灵共轴避弹
    if (ctx.starPos && getDist(ctx.myPos, ctx.starPos) === 1) {
        var dirToStar = directionTo(ctx.myPos, ctx.starPos);
        if (ctx.myDir === dirToStar && isSafeForStarWalking(ctx.starPos, ctx)) {
            // 如果场上没有飞行的真实子弹，或者子弹 1 帧内打不到我们，我们直接不进入规避，走吃星逻辑
...
function isSafeForStarWalking(pos, ctx) {
    ...
    // 1. 【嘴边星星免死金牌】如果我们距离星星只有 1 格，且车头正对，只需 1 帧即可踩星
    ...
    if (!strictSafe) {
        if (canDirectGrab) {
            ...
            if (isAxisSafe) {
                // 在不共轴或超远共轴的安全保障下，只要没有即时子弹会在下一帧内击中星格，且敌人距离星格 >= 1，放行吃星
...
function findSafeGrassSpot(ctx) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        // 【性能剪枝】前置距离过滤，若在 10 格内直接跳过，决不跑 isSafe
...
function findNearestSafeGrass(pos, ctx) {
    ...
    for (var i = 0; i < list.length; i++) {
        var p = list[i];
        // 【性能剪枝】若曼哈顿距离已经不够近，直接 continue，绝不跑 isSafe
        var d = getDist(pos, p);
